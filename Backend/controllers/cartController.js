...
const addToCart = asyncHandler(async(req,res)=>{
    ...
    if(isCart){
        const isProduct = isCart.products.findIndex(pro=> pro.product.toString()=== req.params.id)
        if(isProduct >= 0 ){
        ...
        const product = await Product.findById(req.params.id)
        const resultProduct = []
        resultProduct.push({product:req.params.id, image,version,price,name:product.name})
...
const deleteFromCart = asyncHandler(async(req,res)=>{
    const cart = await Cart.findOne({user:req.user.id})
    const newProduct = cart.products.filter(pro => pro.product.toString()!== req.params.id)
