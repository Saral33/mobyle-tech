...
const getProducts = asyncHandler(async (req, res) => {
  ...
  if (req.query.brand) {
    ...
    res.json({
      remaining:
        totalBrandProducts.length - products.length <= 0
          ? 0
          : totalBrandProducts.length - products.length,
      products,
    });
  } else {
    res.json({ remaining: totalProducts.length - products.length, products });
  }
});
...
const getBrands = asyncHandler(async (req, res) => {
  ...
  products.map((product) =>
    brands.push({ brand: product.brand, image: product.mainImage })
  );

  const result = Array.from(new Set(brands.map((a) => a.brand))).map(
    (brand) => {
...
const createReviewProducts = asyncHandler(async (req, res) => {
  ...
  const reviewdProduct = product.reviews.findIndex(
    (pro) => pro.user.toString() === req.user.id
  );
...
const getStats = asyncHandler(async (req, res) => {
  ...
  const avgStat = await Product.aggregate([
    { $unwind: '$version' },
    {
      $group: {
        _id: '$brand',
        avgPrice: { $avg: { $toDouble: '$version.price' } },
        maxPrice: { $max: { $toDouble: '$version.price' } },
        minPrice: { $min: { $toDouble: '$version.price' } },
      },
    },
  ]);
  const result = productStat.map((e) => ({
    ...e,
    ...avgStat.find((f) => f._id === e._id),
  }));
