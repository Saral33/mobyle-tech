...
const {activateAccountToken} = require('../config/email')
...
const registerUser = asyncHandler(async(req,res)=>{
    ...
    if(isUserExist && isUserExist.verified ){
        throw new Error('User with this email already exist.You may have logged in with google')
    }
    if(isUserExist && !isUserExist.verified){
    ...
    res.json({message:'You need to activate your account. Activation Link has been sent to your email.'})
})
...
const activateUser = asyncHandler(async(req,res)=>{
    ...
    if(user.verified){
        throw new Error('User is already verified. You can login without any problem')
    }
    ...
    if(!user.secretCode || user.secretCode!==token){
        ...
        throw new Error('The link has expired. Link only last 10 minute.New link has been sent again')
...
const getUser = asyncHandler(async(req,res)=>{
    const user =  await User.findById(req.id).select(['-password','-verified','-googleLogin'])
    res.json(user)
})
...
module.exports = {registerUser,loginUser,getUser,activateUser,logoutUser,updateUser,googleLoginUser}