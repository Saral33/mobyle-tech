...
const Products = () => {
    ...
    const {loading, remaining,products,error} = useSelector(state=> state.productsReducer)
    ...
    return (
        <>
         <h2 className='text-center'>Recommendations</h2>   
            ...
            { remaining > 0  &&  <Button onClick={()=> setPage(page + 1)}  variant='primary'>Load More</Button>}
           </Col> 
        </Row>
             }
