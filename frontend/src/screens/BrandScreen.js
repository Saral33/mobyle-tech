...
const BrandScreen = ({match,history}) => {
    ...
    const {error,loading,products,remaining} = useSelector(state=> state.productCategories)
    ...
    return (
        <div className='my-5'>
         ...
         <>
         <Row className='my-4'> {products.map(product=>(
             ...
             <Card.Body>
              <Card.Title as='h5' className='text-center'>
                 <Link className='link' to={`/product/${product._id}?redirect=brands`}>{product.name}</Link>
                 </Card.Title>
                </Card.Body>
              ...
              {remaining > 0 &&  <Button onClick={()=> setPage(page + 1)} className='align-center' variant='dark'>Load More</Button>}
              </Col>
          </Row> 
