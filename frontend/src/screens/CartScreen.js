...
const CartScreen = ({history}) => {
    ...
    const {loading:cartLoading,error:cartError,success} = useSelector(state=> state.cartAction)
   ...
   const checkOutHandler = async()=>{
    setCheckOutLoading(true)
       const lineItem = []
       if(cartItem.length>0){
           cartItem.map((item,index)=> lineItem.push({
               price_data:{
                currency: 'usd',
                product_data:{
                    name: item.name,
                ...
                unit_amount_decimal: Number((item.price *(qty[index])).toFixed(2) * 100),
               },
               quantity: qty[index],
           }))
       }
       ...
       try {
        ...
        const res = await axios.post(`/api/checkout`,{lineItems:lineItem},config)
          ...
          <Row className='pt-5 px-5 mt-5 bg-light'>
            <Col md={12}>
                ...
                {loading? <Loader/>: error? <AlertMessage>{error}</AlertMessage>:
                    ...
                    cartItem.length <=0 ? <AlertMessage>Your cart is empty. Shop first. Your cartItem can be accessed from any device</AlertMessage>
                    :
                    <>
                    <ListGroup className='p-2' variant='flush'>
                        ...
                        {cartItem.map((item,index)=>(
                            ...
                            <ListGroup.Item >
                                <Row>
                                    <Col md={1}>
                                        <Image src={item.image} fluid rounded alt='cart-image'/>
                                        </Col>
                            ...
                            <Col md={2}>
                            <Form.Control as='select' value={qty[index] } onChange={e=>qtyChangeHandler(e,index)} >
                            ...
                            <Col md={2}>
                                {cartLoading? <LoadingButton></LoadingButton>:
                                <Button onClick={()=> deleteHandler(item.product)} type="button" variant="light" >
                    ...
                    <Row className='py-3'>
                      ...
                      <p><strong className='bg-light p-3 text-primary'>
                           Total: ${cartItem.reduce((a,b,index)=> {
                  ...
                  <Row className='py-3'>
                       <Col className='d-flex justify-content-center'>
                           {checkOutLoading? <LoadingButton variant='success'>Proceeding...</LoadingButton>:
                    ...
                    <Row className='py-3'>
                       ...
                       <i className="fas fa-times"></i> Reset</Button>
                       </Col>
                   </Row>
                   </>
                }
