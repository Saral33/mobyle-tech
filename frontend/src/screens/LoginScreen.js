...
const LoginScreen = ({history,location}) => {
    ...
    const {loading,error,authenticated} = useSelector(state=> state.authState)
   ...
   dispatch(googleLoginUser({tokenId:response.tokenId}))
    ...
    return (
        <FormContainer>
            <h2 className='my-3 text-center'><i className="fas fa-lock"></i> Login</h2>
            ...
            <Form onSubmit={submitHandler}>
                ...
                <Form.Group controlId="formBasicPassword">
                    ...
                    <Form.Control minLength={8} value={password} onChange={e => setPassword(e.target.value)} required  type="password" placeholder="Password" />
                </Form.Group>
                <Row className='my-4'>
                 {loading? <LoadingButton>{' '} Loging in.....</LoadingButton> :
