...
const UserProfileScreen = ({history}) => {
    ...
    const {success, loading,error} = useSelector(state => state.editUserState)
   ...
   useEffect(()=>{
    ...
    return (
       <FormContainer>
            <h2 className='my-3 text-center'><i className="fas fa-profile"></i> Update Profile</h2>
        ...
        <Form.Group controlId="formBasicPassword">
          ...
          <Form.Control minLength={8} value={password} onChange={(e)=> setOldPassword(e.target.value)}   type="password" placeholder="Enter Old Password" />
        </Form.Group>
        <Form.Group controlId="formBasicConfirmPassword">
          <Form.Label>New  Password</Form.Label>
          <Form.Control minLength={8}  value={newPassword} onChange={(e)=> setNewPassword(e.target.value)}  type="password" placeholder="Enter New Password" />
        </Form.Group>
        ...
        <Row className="my-2">
            <Col md={4}>
         {uploading ? <Loader/> : <Image fluid roundedCircle  src={image} alt='avatar'/>}   
            </Col>
            <Col md ={8}>
            <Form.Group controlId='image'>
            <label for="formFile" class="form-label">Avatar</label>
            <input className="form-control"  type="file" id="formFile" onChange={imageUploadHandler}/>
            </Form.Group>
            {errorMsg && <AlertMessage>{errorMsg}</AlertMessage>}
            </Col>
        </Row>
         <Row className='my-3 '>
             {loading ? <LoadingButton>{' '} Updating...</LoadingButton>:
